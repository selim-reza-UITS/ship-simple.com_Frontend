...
const AdminShippingConfig = () => {
  ...
  const handleConfigSubmit = (e) => {
    e.preventDefault();
    // Convert percentage inputs back to decimals for the database
    const payload = {
        ...config,
        vat_rate: config.vat_rate / 100,
        margin_rate: config.margin_rate / 100
    };

    api.put("shipping-config/1/", payload)
  ...
  const handleAddCategory = (e) => {
    ...
    // FIX: Divide by 100 so "20" becomes "0.2"
    const payload = {
        ...newCategory,
        duty_rate: parseFloat(newCategory.duty_rate) / 100
    };
  ...
  const handleDeleteCategory = (id) => {
    if (id === undefined) {
        alert("Error: Category ID is undefined. Check if your API returns 'id'.");
        return;
    }
    ...
    <div className="p-8 max-w-6xl mx-auto bg-gray-50 min-h-screen">
      ...
      <section className="mb-12">
        ...
        <form onSubmit={handleConfigSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-white p-6 shadow-sm rounded-xl border">
          <div>
            <label className="block text-xs font-bold uppercase text-gray-500 mb-1">VAT % (e.g. 17.5)</label>
            <input type="number" step="0.1" className="w-full border p-2 rounded" 
              value={config.vat_rate * 100} // Show as 17.5 instead of 0.175
          ...
          <div>
            <label className="block text-xs font-bold uppercase text-gray-500 mb-1">Margin % (e.g. 15)</label>
            <input type="number" step="1" className="w-full border p-2 rounded" 
              value={config.margin_rate * 100} // Show as 15 instead of 0.15
      ...
      <div className="grid grid-cols-1 gap-8">
        ...
        <section>
          ...
          {/* ... Table for rates remains the same ... */}
        ...
        <section>
          ...
          <div className="bg-white shadow-sm rounded-xl border overflow-hidden">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {categories.map(cat => (
                  <tr key={cat.id || cat.name} className="border-b hover:bg-gray-50">
                    <td className="p-3 font-medium">{cat.name}</td>
                    <td className="p-3">{(cat.duty_rate * 100).toFixed(1)}%</td>
                    <td className="p-3 text-center">
            ...
            <form onSubmit={handleAddCategory} className="p-4 bg-blue-50 grid grid-cols-1 md:grid-cols-3 gap-2 border-t">
              ...
              <input type="number" step="0.1" placeholder="Duty % (e.g. 20)" className="border p-2 rounded bg-white" value={newCategory.duty_rate} onChange={e => setNewCategory({...newCategory, duty_rate: e.target.value})} required />
